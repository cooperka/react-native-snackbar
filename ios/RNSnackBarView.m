...
- (void)presentWithDuration:(NSNumber*)duration bottom:(NSNumber*)bottom {
    _pendingOptions = nil;
    _pendingCallback = nil;
    UIWindow* keyWindow = [UIApplication sharedApplication].keyWindow;
    [keyWindow addSubview:self];
    [self setTranslatesAutoresizingMaskIntoConstraints:false];
    [keyWindow addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:[NSString stringWithFormat:@"V:[self(>=48)]-%@-|", bottom] options:0 metrics:nil views:@{@"self": self}]];
...
- (void)show {
    ...
    NSNumber* bottom = _pendingOptions[@"bottom"] ? (NSNumber*)_pendingOptions[@"bottom"] : @(0);
    [self presentWithDuration:(NSNumber*)duration bottom:(NSNumber*)bottom];
}
